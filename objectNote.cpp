...
1.  int addFields(BSONObj& from, std::set<std::string>& fields); /* returns n added */
操作对象是空的bsonObj
...
mongodb的文档不能有重复的键。
文档的键是字符串。
9.查看已有集合，show collections 
10.可以不用提前创建集合：当你插入一些文档时，mongodb 会自动创建集合。例：
...
不想指定查询条件参数 query 可以 用 {} 代替，但是需要指定 projection 参数：
eg:
querydb.collection.find({}, {title: 2})

以易读的方式来读取数据，可以使用 pretty()
>db.col.find().pretty()
...
例：
db.tk.player_collection.find({$or:[{"Band.sl":3},{"Band.sl":2}]},{"Band":1}).pretty()
