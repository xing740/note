...
循环使用照成内存泄漏:
在Man类内部会引用一个Woman，Woman类内部也引用一个Man。当一个man和一个woman是夫妻的时候，他们直接就存在了相互引用问题。man内部有个用于管理
wife生命期的shared_ptr变量，也就是说wife必定是在husband去世之后才能去世。同样的，woman内部也有一个管理husband生命期的shared_ptr变量，也
就是说husband必须在wife去世之后才能去世。这就是循环引用存在的问题：husband的生命期由wife的生命期决定，wife的生命期由husband的生命期决定，
最后两人都死不掉，违反了自然规律，导致了内存泄漏。

原文链接：https://blog.csdn.net/shanno/article/details/7363480
...
1.lexical_cast在转换字符串时，字符串中只能包含数字和小数点，不能出现除e以外的英文字符或者其他非数字字符。
...
3.接受一个参数的构造函数允许使用赋值句法初始化对象。   所以当类的构造函数是一个参数时，有时要加expliclt,避免进行隐式转换
例：class c {
    private:
        int x;
    public:
...
反码：正数时与原码相同，负数时是除符号位外按位取反
补码：正数时与原码相同，负数时是反码加1
负数的二进制，转成十进制是负数的绝对值=补码
eg:1111 1111
...
172.当要描述一个物体有多冲描述方法，但一个对象只会有一种描述时，可用union。例：
    struct base
    {
      bool rare;
    };
...
不可内存操作的调用相应的函数。为了能在编译时决定调用什么函数，函数以重载的方法实现，所以是否能内存操作的标识不能只是一个类型，
需是一个参数.
...
309.使用non-member函数的另一种好处,方便一些写法
...
move对于拥有如内存、文件句柄等资源的成员的对象有效，如果是一些基本类型，如int和char[10]数组等，如果使用move，仍会发生拷贝（因为没有对应的移动构造
...
例：1
A a1 = GetA();a1是左值，GetA()先构造出一个临时对象，再调用A的拷贝赋值函数进行构造a1，临时对象再调用构造函数进行构造
A&& a2 = GetA();将调用A的移动赋值,将GetA()产生的临时对象的的地址复制过来，再将GetA（）的对象地址置空
...
328.remvoe_reference 去掉引用，返回纯粹的类型,提交出来的类型的别名是成员变量type
...
328.std::forward 如果是右值，不做改变，如果是左值，则返回右值
...
342.unordered_map是计算key的哈希值，以找回value存放的对应格子,修改value不需重新插入,map需重新插入

342.函数形参是引用时，不能传临时变量，是const的引用时，可以传临时变量


345.用内存池的原因：
c/c++的内存分配（通过malloc或new）可能需要花费很多时。更糟糕的是，随着时间的流逝，内存将形成碎片,所以一个应用程序的运行会越来越慢。
当它运行了很长时间或执行了很多的内存分配（释放）操作的时候。堆（heap）会变成碎片的。
...
347,./可执行文件名 参数1 ...
int main(int argc, char* argv[]) argv[0] 是函数名，其后是参数
一个程序的执行必须有main函数入口，且不能被其它函数调用，所以当需要参数时，不能在代码中取的，只能从外传入，因此main函数有参数
exec不能看成只执行函数，它是执行可执行文件的

348.std::map lower_bound下限。 排序小到大
mp[1] = "a";                                                                                                                                                                       
mp[3] = "b";                                                                                                                                                                       
mp[5] = "c";                                                                                                                                                                       
mp[7] = "d"; 
mp.lower_bound[3].first == 3  //找出第一个>=3的
mp.upper_bound[3].first == 5  //找出第一个 > 3的
mp.lower_bound[4].first == 5
mp.upper_bound[4].first == 5
...
例：有10个数据，每次最多只能读2个数据，读完后就又返回epoll_wait监听，又会触发该fp
...
read的过程：
read时，会调用底层的api(就是系统调用),这时会转为内核态，1.内核会负责将网卡中的数据复制到内核空间(需要一个过程的,应为可能数据
  还没完全到达，没收到一个完整的包，这时就没被拷贝到内核的缓存,)，2.再负责将内核中的数据复制到应用空间.最后用户进程才解除block状态，
重新运行起来。

正式因为1,2过程用户进程会block，linux系统产生了下面五种网络模式的方案。
...
wiki   /data/game-wiki/xl_moin-1.9.9/wikiserver.py       screen -S xxx
...
361.List迭代器失效
1.向list中插入元素，不会使迭代器失效
2.当从list删除一个元素后，指向该元素的迭代器失效。其它迭代器不受影响。所以可用后++遍历删除，for(auto itr = xx.begion(); itr != xx.end(); itr++)

362.inline
编译器有个原则，以cpp文件为单位进行逐个编译obj，每个cpp文件的编译是独立的，该cpp文件用到的外部函数都在编译时预留一个符号,只有等到所有obj生成后链接时才给这些符号地址（链接脚本决定地址），所以其他cpp文件编译时只会看到这个函数的声明而无法知道她的实体，链接的时 候再填入该地址，做不到内联展开。 所以要内联则必须在每个用到它的cpp文件体现实体，那就只有在头文件了，类体内实现的函数， 默认是inline的，inline函数在类体外实现时，实现需在头文件中。
...
364.::xx 无名命名空间，表示xx是全局中的一个变量

368.一个函数能访问外部变量就是闭包

371.想使得单个基础数据类型在多线程中安全,尽量使用原子操作代替锁机制(atomic). 当需要对代码块进行安全操作时,就需要选择使用锁机制了.

372.future,异步操作线程返回值。
函数async()用于产生future对象，它异步地启动一个线程运行函数，返回future对象，这样随后我们就可以利用future获取
计算结果。
例：auto f = async(fab, 5); fab是函数 int fab(int n)

374.C++标准规定，当一个指针类型的数值是0时，认为这个指针是空的。在初始化列表中，可以将指针值置0表示将指针初始化为nullPtr

374.ipp 用于将模板的声明与实现分开，因为模板是在预编译时进行处理的，所以不能放到.cpp文件里面，所以就定义了.ipp文件。 
将模板的实现部分放到.ipp文件，然后在模板声明的.h文件后面 #include 这个.ipp文件

377.cpu是16位，表示在cpu内部一次性处理、传输、暂时存储的信息最大长度是16位，但地址总线不一定是16,最大寻址能力是根据地址总线数定的,
地址总线能代表的最大的数，就是能寻址到的内存数

379.map
自实现排序：
map 比较相等的方法， a.x < b.x 返回假，且，b.x < a.x 返回假，则表示两数相等，只用到了重载小于号，没必要多加一个operator==
1.第三个参数是一个可实现仿函数的类，可定义一个类，实现类的访函数，参数是两个。
struct Cmp
{
...
2.如果key是结构体，在结构体中实现重载小于号的函数。每个值都要比较，不然用一个key去查找时，会找不到。
...
384.double不能精确的表示很大的数
double 浮点数结构如下:
1 位符号位
11 位指数位
52 位尾数位      使用52位表示一个数的整数部分，所以当发一个整数是用63位表示时，就会不对

387.0b表示的是2进制，0b1 << idx, 应该0b1分配的是int类型，所以idx不能超过31

388. #define   #的使用，#相当于把后面的参数字符串化，即前后加上""

389.定时器：在指定内存开辟一定的空间，将指针加入qtimercollection中，设置定时器的事件id、时间、回调函数。
...
390:
<< 1 相当于*2，>>1 相当于/2   例：int x = 4; x << 1; x不会被修改

391:成员变量声明时不能用括号进行初始化，可在初始化列表中使用
...
393.拉当前mg   /data/sh db_lq. sh id 加要放在哪个目录里
1.ssh root@49.234.131.111  数据   own的都不在这个机中
2.ssh root@49.234.133.67   as

395.变量前面加（void）是什么作用？
是一种伪装手法，对程序没有影响.如果一个变量定义了，但是没有被后续代码使用，编译器就会给出警告，使用(void)a可避免之。

396. boost::addressof
获取基本变量、类变量、函数的地址，避免因代码中实现了&的重载函数而照成错误。
eg1:对一个指针变量使用addressof，是取的这个变量的地址，不是这个变量上存的地址数据

398.
frpc
1.网页和ssh都可以选择tcp,基本设置中，只填本地ip和端口，意思是这个地址映射到外网的某个地址上。
2.frpc_linux_amd64 -f nf8zk1pcjbhoi25t:923040  在本地ip的机子上运行这个服务，即开始了映射。
会得到下列提示。 []中的地址就是映射后的地址，访问那个地址的信息，都会转发到内网的ip:port上。
例：ssh xing@35.160.49.136 -p58173  会发到内网，即ssh xing@本地ip

TCP 类型隧道启动成功
使用 [us-or-aws.sakurafrp.com:58173] 来连接到你的隧道
或使用 IP 地址连接（不推荐）：[35.160.49.136:58173]
...
401.因为网络id和主机id的位数不固定，可能相互占用，所以出现了子网掩码

402.时间复杂度，logn 以2为低数，n为真数，eg:n为4时，logn等于2,二分法就是logn
...
34.return 可不执行断点以下的代码，并返回

35. p *((playerItem*)0x7fad322ccab0)

36.bt打印出堆栈后，用f编号跳入指定的堆栈信息
...
注意unset这个东西只认key    value可以是任意的.true.1或者其他都没关系.只要看到 allyreq就会干掉它

mongoep:
1.$addToSet添加值到一个数组中去，如果数组中已经存在该值那么将不会有任何的操作。
...
> db.mycol.aggregate([{$group : {_id : null, num_tutorial : {$sum : 1}}}])
...
4. which 查找可执行命令在哪个目录，例: which ls

5. 每个命令执行后都有一个返回码设置到 ？ 中，可用 echo $? 查看，0表示返回成功

6. !!执行上一个命令，！num执行history中的某个指令
...
foo.c.r30
foo.c.r31
 svn resolve --accept working 1.json
...
svn 回滚
1.linux 的  svn up -r 10 和  window 的   update to revision,都是可以将本地的文件改到显个版本，但用commit无法上传变化，update 后会
恢复到最新
2.linux svn merge -r 20:10 [文件或目录](必须要有文件或目录), 就是将20版本的内容变成10版本的内容,此时用commit，即将10版本的内容变成最新版本内容。
window是 先查看log，选中要恢复的版本，revert to this revision，则可将本地变成某个版本，commit，则最新版本内容变成这个版本内容

————————————————————————————git——————————————————————————————————————
1.提交一个新的目录到仓库
进入目录执行git init  表示将该目录初始化为一个git的工作空间
...
2.因为有多个线程可以调用run，为了避免竞争，加了锁。在调用io_service的接口时，在真正的处理前，都要去夺取锁，谁先夺到，哪个线程就负责处理。
3.调用post等处理handle时，直接将任务放入任务链表，唤醒一个空闲线程进行处理，若当前没有空闲线程，就创建一个立即返回的事件加入
...
2.debug和release区别
 ...
 4.operator 除操作符重载外，还是自定义对象类型的隐匿转换(对象会自动调用转换)
 class a
 {
 public:
...
1.将1都改成 5， c-v > r > 5 
2.在顶部插入数据 移到顶>c-v > I(大写) > 输入数据
3.在尾部插入数据 移到尾>c-v > A(大写) > 输入数据
...
python
0.range(5):创建一个队列，最大值小于5，默认从0开始，步长1，即[0,1,2,3,4]
...
2.yield 对应的值在函数被调用时不会立刻返回，而是调用next方法时（本质上 for 循环也是调用 next 方法）才返回
3.每次调用一次next，函数都会执行到yield处停止，等待下一次next，next次数超出限制就会抛出异常
...
2020-3-13
深入探讨c++对象模型
1.类中的函数生成多个实例，并不会产生多份，在调用函数时，会用堆栈去存取使用的函数的地址，使用函数产生的变量。函数的代码只会被读，并不会改变，所以
可有多个线程同时对其进行操作？

2.类中变量分为：成员变量和静态成员变量，其中成员变量存储在类中，函数分为：函数成员、静态函数成员，虚函数，只有虚函数表指针存在类中。

3.指向不同类型的指针间的差异，既不在其指针表示法不同，也不在其内容不同，而在其所寻址出来的对象类型不同，也就是说，
“指针类型”会教导编译器如何解释某个特定地址中内存内容及其大小.所以基类的指针只能找到基类的数据，派生类的指针能找到
其身和基类的数据，基类指针想操作派生类的数据，只能通过虚函数

4.void*指针没有类型，无法知道它所涵盖的空间，所以void*指针只能持有一个地址，不能通过它操作指向的对象。

6.Global objects的内存保证会在程序启动的时候被清0,Local objects配置于程序的堆栈中，heap objects配置于自己空间中，
都不一定会被清0，它们的内容将是内存上次被使用后的遗迹。

7.当类内有成员对象时，类的系统默认构造器，会自动的加入成员对象构造的代码，如果程序员实现了类的构造器，系统
...
8.默认构造器只有在编译器认为有需要时，才会被合成出来，默认构造器只会负责初始化基类和成员对象。

9.默认拷备构造函数也是在编译器认为需要时才被合成出来，默认拷备函数发生的是位拷备。但在继承中，若基类中有虚函数，
当将派生类的指针赋值给基类的指针时，类中的虚函数表指针发现的不是拷备，虚编译会使基类中的虚函数指针不是指向基类的
虚函数表的指针。
...
13.一个空类生成对象时，它有一个隐藏的1 byte大小，是被编译器安插进去的一个char,这使的生成的对象在内存中配置独一无二的地址。

14.
class x {}; //空类，size 1
class y : public virtual x {}; //虚函数表指针 + 继承的1 = 5，边界调整 size 8   存在一个虚函数表指针，所以不算空类
class z : public virtual x {}; //虚函数表指针 + 继承的1 = 5，边界调整 size 8
class A : public y, public z {}; //因为x是被虚继承，所以x,y,z都共享一个class实例，所以A size = x大小1 + y内虚指针4 + z内的虚指针4，边界调整 12
不能理解成 A 中 y z 先边界调整后,再传给A，边界调整是对最后的那个派生类而言的。

15.静态成员函数不能被const修饰，因为静态成员函数不能直接访问成员变量，所以没必要声明成const
15.多态int
int main ()
{
    class A {
...
1.如果将 a = b;则发现的是拷贝构造，默认的构造函数是位拷备。并不存在指针类型的转换，所以不存在多态，但如果简单的将 b中虚函数表的指针赋值给
a 的虚函数表的指针，a调用虚函数时就会出错，所以编译器会对这种情况进行处理，赋值后，虚函数表的指针指向A,调用时打针出 "is A"
2.当是指针或引用类型进行赋值时， A* a = b; 则地址会进行类型转换，b中原本的虚函数指针，会被转成A类型的指针，所以只能寻址到A类虚函数表中存在
的函数。  下列16的解释应该也正确
3.多态是运行时才决定调用哪个的实例的，但并不是根据指针的类型去决定的，每个虚函数都有固定的slot，，传入相应的slot，得到的函数实例是
什么就是什么。
...
17.虚函数的调用过程
ptr->normalize();
假设normalize是虚函数，内部将转化为 （*ptr->vptr[1])(ptr);
vptr是虚函数表的指针，虚函数表指针也会经过 mangled（倾压),因为一个class中可能含有多个vptrs
1是normalize在表中的索引值
ptr是this指针

18.成员函数指针
1.普通函数指针，只需确定返回值和参数类型，就能指向某一类函数。格式：返回值类型 (* 指针名，可随便取)(参数类型,...)  例：int (*pf)(int a)
2.取一个nonstatic member function的地址，且是非虚函数，得到的是它在内存的真正地址，然而这个值是不完全的，它需要被绑定于某个class object的地址上。
格式：
double       返回值类型
(Point::*    标识是成员函数
  pmf)       指针的名字
...
19.一个对象在构造函数执行完之前，它并不是一个完整的对象


makefile
1. make xx.c  可编译.c文件
2. make 不知道各文件的依赖关系，所以需要makefile知道各文件的依赖关系和编译规则。


设计模式
1.工厂模式
1.简单工厂：只有一个工厂类，根据传入的参数决定创建哪个对象(或某个对象执行操作,不一定得是标准的工厂模式)
...
2.工厂方法模式  不再提供一个统一的工厂类来创建所有的对象，而是针对不同的对象提供不同的工厂。也就是说每个对象都有一个与之对应的工厂。
...
3.抽象工厂模式：  多个对象对应一个工厂，一个工厂调用不同接口可生产不同产品。对象可有不同的抽象类
缺点：增加一个新对象要改动多处代码
...
2.建造者模式  建造者模式就是如何一步步构建一个包含多个组成部件的对象，相同的构建过程可以创建不同的产品
场景：
...
场景:
1.当一个对象有多个变化因素的时候，考虑将对象依赖于抽象的实现，而不是具体的实现。如电脑有2种变化因素，一个是品牌，一个是操作系统
将品牌和系统抽象出来，创建某个品牌的电脑，将对象的系统做为参数传入。
2.如果不希望在抽象和它的实现部分之间有一个固定的绑定关系，也就是继承关系；如果我们打破了这种固定的绑定关系，以后，就可以方便的
在抽象部分切换不同的实现部分；
3.如果一个对象有多个变化因素的时候，通过抽象这些变化因素，将依赖具体实现，修改为依赖抽象；

缺点：
1.需要知道某种品牌的电脑对象什么操作系统
...
4.装饰模式  动态地给一个对象动态的添加一些额外的功能，就增加功能而言，装饰模式比生成子类更为灵活。
场景：
1.当系统需要增加新功能时，如果向旧的类中添加新的代码，通常这些代码是装饰了原有类的核心职责或主要行为，它们在主类中加入了新的字段、新的
...
5.  组合模式：将对象组合成树形结构以表示“部分-整体”的层次结构。Composite使得用户对单个对象和组合对象的使用具有一致性。
场景：
1.你想表示对象的部分-整体层次结构
2.你希望用户忽略组合对象与单个对象的不同，用户将统一地使用组合结构中的所有对象。

6.享元模式  类似一个工厂类管理多个单例。
#include<iostream>
#include<string>
#include<vector>
using std::cout;
...
6.代理模式  为其他对象提供一种代理以控制对这个对象的访问。 在需要用比较通用和复杂的对象指针代替简单的的指针的时候，使用代理模式。有四种常用的情况：
场景:
...
4、智能指针，取代了简单的指针，它在访问对象时执行一些附加操作。
 ...
 7.策略模式  它定义了一系列的算法，并将每一个算法封装起来，而且使它们还可以相互替换。策略模式让算法的变化不会影响到使用算法的客户,客户自主选择算法(策略)
...
8.状态模式  允许一个对象在其内部状态发生变化时改变自己的行为， 就好像是更换了一个不同的类一样
场景：
1.一个对象的行为依赖于其内部状态， 并且它必须在运行时刻根据自己的状态改变行为。
2.多个操作存在大量的， 多个部分的依赖于对象状态的条件语句。 而这个状态一般用一个或多个常量表示。 一些操作包含相同的的条件判断结构。
...
scp xing@10.17.172.222:/data/lsfz_test_s001a/server/svr_source/world_game/wgg ./



as  49.234.133.67  /data/lsfz_qixingtan_s001 
49.234.157.20  体验服
...
隧道名: hhh
映射类型: tcp
启动参数: frpc -f nf8zk1pcjbhoi25t:205045

问题：

zhanche:
问题：
1.没看到game中一开始是怎么连gate，又怎么启动gm_svr

1.gate中启动两个服，一个是客户端连接(eg:gac_gate，一个是负责内部连接(eg:game-gate)
2.收到gac的数据后，如果是发给game，就从内部服中找game的io


javascript
0.配置
1：开发环境，写代码开发的。生产环境，外网的
2:npm
...
3.在js中，函数可看成对象，对象即是函数，因为js中的函数可以有属性和方法。ts会被解析成js，所以ts中的类，最终会解析成函数。箭头函数
没有this，不适合定义对象方法。
...
4.promise
0.能方便的实现链式操作
1.是一个构造函数，new promise(fun(resolve, reject))，会执行这个构造函数，返回一个promise对象,由构造函数可知，传入的执行函数要有两个参数，
...
2.promise中不是自动异步，需要有异步的实现，异步中可调用resolve或reject
3.promise.all([]),可传入一些异步，等所以异步完成后，才会执行then，then函数参数接收到的参数是一个数组。
4.这里的回调函数，不存在于promise的参数中，而是resolve和reject的参数
5.promise中不调用reslove，则then不会执行，catch一样。
6.传递到 then() 中的函数被置入到一个微任务队列中，而不是立即执行，这意味着它是在 JavaScript
事件队列的所有运行时结束了，且事件队列被清空之后，才开始执行。(意思是then后的代码会先于then执行，置于then中的代码是不是异步，就不知道了)
7.Promise有三个状态，分别为pending、fulfilled和rejected，且只能从pending到fulfilled或者rejected，所以promise返回的promise是新的，是
pending状
8.then方法中会返回新的promise
9.eg:
console.log(1);//主
...
以属性的形式使用:var tm = person.time;  person.time = 0;(赋值时也是以属性的形式);
...
9:项目io建立。
gate启动时，在main中创建gate_server,设置监听标识connect，有gac连接后，会提到一个io，也设置这个io的标识

10.WebSocket  一种网络通信协议
1.WebSocket 连接允许客户端和服务器之间进行全双工通信，以便任一方都可以通过建立的连接将数据推送到另一端。WebSocket
只需要建立一次连接，就可以一直保持连接状态。这相比于轮询方式的不停建立连接显然效率要大大提高。
2.



1.如果把要给数值放入引号中，其余数值会被视作字符串并被级联。(与字符相加的数字会被当成字符)
eg1: var x = "8" + 3 + 5;  先算 “8” + "3"  再算 "83" + "5"
eg2: var x + 3 + 5 + "8";  先算 3+5 再算 “8 + “8"
eg3:除了加号，其它运算符号，都会把字符串数字转成数字进行运算
var x = "100";
var y = "10";
var z = x - y; = 90

2.对象也是变量。但是对象包含很多值。除了原始值（没有属性或方法的值,eg:string,nubmer....)，都是对象
eg:var car = {type:"porsche", model:"911", color:"white"}; //type可看成是变量名
访问对象属性:
1:objectName.propertyName
2:objectName["propertyName"]//如果[]中传入一个对象，js会隐式的调用toString(), ts则不会，会提示出错
访问对象方法:
...
3.如果通过关键词 "new" 来声明 JavaScript 变量，则该变量会被创建为对象：
var x = new String();        // 把 x 声明为 String 对象
var y = new Number();        // 把 y 声明为 Number 对象
var z = new Boolean();       // 把 z 声明为 Boolean 对象
请避免字符串、数值或逻辑对象。他们会增加代码的复杂性并降低执行速度。

2.元组和数组的区别：元组能存不同的类型
3.原始数据类型
string
number
boolean
...
5. === 运算符需要类型和值同时相等。 == 值相等。  对象无法进行比较

6.方法和属性也可用于原始值，因为在执行方法和属性时 JavaScript 将原始值视为对象。
eg:
var txt = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
var sln = txt.length; //此时的txt是对象

7.valueOf() 方法可将 Number 对象转换为原始值。
...
9.比较
eg1:数字与数字字符进行比较时，会先把数字字符转成数值,空字符转成0，   2 < "12" true
eg2:数字比非数字非空字符进行比较，结果都是false  "2 > "John" false
eg3:字符串进行比较时，比较第一个字符的assic值 "2" > "12"  true

10.Switch case 使用严格比较（===）

12.如果您为尚未声明的变量赋值，此变量会自动成为全局变量。
eg:
myFunction();
function myFunction() {
    carName = "porsche";
...
13.Hoisting 是 JavaScript 将所有声明提升到当前作用域顶部的默认行为（提升到当前脚本或当前函数的顶部）。
所以变量可以先使用再声明. 若声明的同时初始化，则不会被提升

14."use strict" 严格模式
eg1:您无法，例如，使用未声明的变量。
eg2:在严格模式下，this在函数中使用时，this 是未定义的（undefined）

15.call() 和 apply() 方法是预定义的 JavaScript 方法。
...
eg2:类型常量指针，指针的指向不可变，但指针指向的值可变
eg3:通过 const 定义的变量不会被提升到顶端。 const 变量不能在声明之前使用：

18.函数的参数是值传递，对象是引用传递

19.当不带拥有者对象调用对象时，this 的值成为全局对象。
eg1:调用全局函数
...
20.如果函数调用的前面是 new 关键字，那么这是一个构造函数调用。会创建一个新对象
eg:
// 这是函数构造器：
...
vscode
1.task.json 类似于写好一个脚本，需要的时候就调用执行
2.launch.json 启动调试时，vs从里面找要执行文件的信息，写好launch后，不论选中什么文件，都按照launch的规则执行
launch是启动程序并进行调试；
attach是调试某个已启动的线程；
...
npm:
1.npm init 会生成package.json,使用npm intall安装套件时，会记录到package.json中，
2.npm install会先检查node_modules目录中是否存在指定模块，如果存在就不安装，即便远程有新版本。如果.npm中有压缩包，但没有安装到node_modules中，也会
重新下载
3.
发出npm install命令
...
typescript
问题：
1.TypeScript使用的是结构性类型系统。 当我们比较两种不同的类型时，并不在乎它们从何处而来，如果所有成员的类型都是兼容的，我们就认为它们的类型是兼容的。
...
5.类型断言：告诉编译器这是什么类型，以实其能过编译，使用时，是不是断言时的类型，就需要程序员控制了。
eg:
interface Cat {
    name: string;
    run(): void;
...
2.类
1.继承时，派生类中有构造器时，必然包含super()
eg1:类中的方法不能等同于函数，类中方法不需要加function
 
3.TypeScript里的类型兼容性是基于结构子类型的。 结构类型是一种只使用其成员来描述类型的方式。 它正好与名义（nominal）类型形成对比。
...
7.接口 :接口的作用就是为这些类型命名和为你的代码或第三方代码定义契约(有哪些变量名，和变量的类型),只声明成员方法，不做实现
...
eg4:混合型接口, 一个对象可以同时做为函数和对象使用，并带有额外的属性。
interface Counter {
    (start: number): string;
    interval: number;
    reset(): void;
...
7.类
eg1:构造函数的形参被修饰时，表示??是一个成员变量
constructor(readonly name: string)
constructor(public name: string) 

8.抽象类 :做为派生类的基类使用。 它们一般不会直接被实例化,与接口的不同之处是成员可以有实现细节
...
链接：https://juejin.cn/post/6857123751205535751
...
eg2:类的私有成员和受保护成员会影响兼容性。 当检查类实例的兼容时,
...
13.异步
1.promise里放方法,方法中可以放两个回调函数(目的是方法异步执行后，将结果返回)
eg:
...
2.then 可接受两种参数。 then(resolve=>{},reject=>{})或.then(resolve=>{}),即resolve必须传，reject可选择
3.catch 等同于 then(null,reject=>{})
4.async 的方法就可以看做一个封装好的 Promise 对象，调用该方法就会立刻得到一个 Promise 对象（所以async修饰后的方法可以不用再加promise,
该方法返回的值是 then 的成功回调的参数；当需要对一个 async 方法或者返回 Promise 对象的方法进行异步等待时，就要加上 await 关键字，
同时加了 await 关键字的方法要变成用 async 修饰的方法； 如果不加 await 关键字，就要使用 then 方法来监听异步的回调
5.TS下的Promise附带了一个泛型类型，如下Promise<T>，这里的T表示会传递给resolve的参数类型。
6.await:
在调用任何返回Promise的函数前使用 await. promise状态变为fullfilled后继续往后执行
7.优化：过多使用await会使程序变成单线程，可用用变量存返回的promise，返回后再使用
eg:
async function timeTest() {
  ...
  await timeoutPromise1;//看来await也可以用在promist对象前面
  await timeoutPromise2;
  await timeoutPromise3;
}

8.索引签名，可理解成索引的认证(eg:索引的类型需是什么)，这里表示对象的索引。
1.ts的索引需是string或number;
2.声明一个索引签名
eg: const foo:{//声明对象的索引(idx)是string类型，值是对象，是｛message:string}类型
  [index: string] : {message: string};
} = {};
声明成index名只是为了可读性，可声明成其它
3.声明一个索引签名时，所有明确的成员都必须符合索引签名：
eg:
// ok
interface Foo {
...
9. < T extend x> 约束T的类型为x类型


面向对象：
1.采用里氏替换原则
父n类使用的地方，可以用子类替换，所以不要覆写父类已有的方法.需要重载父类已有的方法，子类的方法要比父类的苛刻，比如参数或返回值。
2.依赖倒置 https://zhuanlan.zhihu.com/p/24175489
...
3.一个方法不知道放在哪个对象中实现时，可从依赖哪个对象的变量比较多处判断
4.阵营战，进入城市或道路时，都会创建一个进入的对象，但不进行保存，位置信息保存在玩家的位置中

5.家园，每个建筑对象都有升级操作，但是是某些建筑升级操作相同，所以可以把升级的所有操作封装成一个对象，建筑需要哪一类的升级操作，就包含哪个对象
...
1.不要关注过去的原因，而是现在的目的,很多情绪和行动，是由你当前的目的照成的。如果依赖原因论，会一直止步不前。
2.决定我们自身的不是过去的经历，而是我们自己赋予经历的意义。人生不是由别人或经历赋予的，而是由自己选择的，是自己选择自己如何生活，
所谓的心理创伤，事实上是我们从过去的经历中去寻找符合自己目的的因素。
3.不可能乘坐时光机回到过去，也不可能让时针倒转。如果成为了原因论的信徒，那就会在过去的束缚之下永远无法获得幸福。结果是陷入对世界的绝望，
对人生的厌弃的虚无主义和悲观主义之中。
4.重要的不是被给予了什么，而是如何利用被给予的东西。一直想变成其他人，就是因为一味的关注被给予了什么。而应该把注意力放在如何利用被给予的东西，
努力把自己变成想成为的自己。所以现在的幸或不幸，都是自己选择的。
5.一直厌弃自己，一直想努力改变，但却一直没变，是因为我们不断地下着不改变自己生活方式的决心.一直保持现在的我，那么如何应对眼前的事情以及其结果会怎样等
问题都可以根据经验进行推测，可谓是轻车熟路般的状态。如果选择新的生活方式，未来难以预测，生活就会充满不安，也可能有更加痛苦，更加不幸的生活在等着自己。
也就是说，即使人们有各种不满，但还是认为保持现状更加轻松，更能安心。要改变需要勇气。所以说阿德心理学就是勇气心理学。不幸是因为缺乏获得幸福的勇气。
6.如果怎么会怎样。例如，如果我有时间，如果我出身等等这些，正是自己为了不做改变而找的借口。想做的事要勇敢的去做，不管结果
如何，我们或许会有所成长，会明白一些事情，总之，一定会有所发展。所谓改变现在的生活方式就是如此。
7.目的论是说：无论之前的人生发生过什么，都对今后的人生如何度过没有影响。决定自己人生的是活在此时此刻的你自己。
8.讨厌自己是因为下了不喜欢自己的决心，其实这是一种自己对自己的善。比如不去表白，就是怕被拒绝后，会对自己更没自信，更讨厌自己，所以找一个自己害羞等的借口，
一直活在想像之中,如果我不害羞，就会怎样怎样，以此来避免自己受到伤害。
9.一切烦恼都来自人际关系。害怕在人际关系中受伤。但在社会中生存，就一定会有不喜欢你的人，一定会受伤，所以就把自己封了起来，尽量不去触及人际方面的事情。 
因害怕受到人际关系伤害这个原因，而把自己变成一个独处的人，为了支持独处这个目的，找出很多个自己的缺点的理由，进而更讨厌自己，再加自卑，再加害怕
交际，再加喜欢独自。
10.对自已某一方面不满意，而更加努力，不断进步，这种自卑感是正常的。病态的自卑感觉是什么都不做就否定自己，没有往前迈进的勇气，这是一种自卑情结。
11.很多时候我们是不想成功:害怕向前迈进或是不想真正的努力。不愿意为了改变自我而牺牲目前所享受的乐趣。也是是拿不出改变生活方式的勇气。即使有些不满或者不
自由，也还是更愿意维持现状。
12.没有人能长期忍受自卑感。健全的姿态应该是通过努力和成长弥补的部份。没有勇气的人就会陷入自卑情节，因为我欠缺这方面，所以做不到另一方面。
又因为忍受不了，就又会产生如果怎样，我会怎样的想法，这就一步步走到了优越情节方向。通过一些简单粗爆的东西来表现自己很优秀，继而沉浸在一种
虚假的优越感之中。比如权势张扬，穿名牌，沉迷于过去的荣光等。另一个异常优越感是通过表现自己特殊来实现，夸耀不幸，这一切都是想得到别人的认可，关注。
13.健全的自卑感不是来自与别人的比较，而是来自与理想的自己的比较。应该积极看待自己和别人的差异。我们虽然不同，但平等。我们不断前进不是为了于人
竞争，价值是不断超越自我。
14.如果一直把别人看成竞争对手，就会不断的比较，产生优于这个，输于那个的想法，自卑情结和优越情结也会随之产生。更甚者会把别人看成敌人，随时会愚弄、嘲讽自己。
所以会认为人人都在关注自己，其实在意你脸的，只有你自己。
15.无法真心祝福过得幸福的他人，是因为站在竞争的角度考虑人际关系，把他人的幸福看作我的失败，所以才无法给予祝福。
16.基本上，一切人际关系矛盾起因于对别人的课题妄加干涉或自己的课题被别人妄加干涉。只要能够进行课题分离，人际关系就会发生巨大改变。
17.太在意别人的视线和评价，所以才会不断寻求别人的认可。人太在意别人的评价的原因是，还不会进行课题分离，把原本是别人的课题也看成自己的课题。
18.如果人际关系中有回报思想的存在，那就会产生因为我为你做了这些，所以你就应该给予相应的回报这样的想法。你是一种与课题分离相悖的思想。我们既不可以寻求回报
也不可以受其束缚。
19.选择了不自由生活方式的大人，看着自由活在当下的年轻人就会批判其享乐主义。这其实是为了让自己接受不自由生活面捏造出的一种人生谎言。
20.不想被人讨厌，是本能性的欲望、冲动性的欲望。这种生活方式只是欲望和冲动的奴隶。八面玲珑地讨好所有人的生活方式是一种极其不自由的生活方式，同时也是不可能
实现的。如果要行使自由，那就需要付出代价。而在人际关系中，自由的代价就是被别人讨厌。毫不在意别人的评价，不害怕被别人讨厌，不追求被他人认可。
21.不想被人讨厌也许是我的课题，但是否讨厌我却是别人的课题。即使有人不喜欢我，我也不能去干涉。
22.因为不想与父亲和好，所以才搬出被打的记忆。当下定修复关系的决心时，父亲拥有什么样的生活方式，怎么看我，对我主动靠近他这件事持什么态度等，这
些都与我毫无关系了。即使对方根本不想修复关系也无所谓。问题是自己有没有下定决心，人际关系之卡，是掌握在自己手中的。选择满足他人希望的生活方式，就是把
人际关系之卡放到了别人手中。
23.提到人际关系，人们往往会想起两个人的关系或者与很多人的关系，但事实上首先是自己。如果被认可欲求所束缚，那么人际关系之卡就会永远掌握在他人手中。
24.不能进行课题分离，一味拘泥于认可欲求的人也是极其以自我为中心的人。你正因为不想被他人认为自己不好，所以才在意他人的视线。这不是对他人的关心，而是对
自己的执著。甚至会认为。大家都应该为我服务，应该优先考虑我的心情。
25.归属感不是仅仅靠在那里就可以得到，它必须靠积极地参与到共同体中去才能够得到。就是直面人生的课题。也就是不回避工作、交友、爱之类的人际关系课题。要
积极主动地去面对。如果你认为自己就是世界的中心，那就丝毫不会主动融入共同体中，因为一切他人都是为我服务的人，根本没必要由自己采取行动。但是，无论是你
还是我，我们都不是世界中心，必须用自己的脚主动迈出一步去面对人际关系课题，不是考虑这个人会给我什么，而是要必须思考一下，我能给这个人什么。这就是
对共同体的参与和融入。归属感不是生来就有的东西，要靠自己的手去获得。
26.帮助别人时帮助他人用自己的力量去解决。也就是可以把马带到水边，但不能强迫其喝水。直面课题的是其本人，下定决心的也是其本人。人害怕面对课题并
不是因为没有能力，纯粹是缺乏直面课题的勇气。首先应该找回受挫的勇气。勇气不能通过受到表扬获得，人会因为被表扬而形成自己没有能力的信念。如果生活中
总是某求别人的表扬，最终就会选择迎合他人价值观的生活方式。
27.不评价他人，评价性的语言是基于纵向关系的语言（注意观察自己平时与感觉平等的人之间和不平等的人之间的对话可见一般)。如果建立起了横向关系，那自然就会说出一些更加真诚地表示感谢、尊敬或者喜悦的话。人在听到
感觉之词的时候，就会知道自己能够对别人有所贡献。人只有在体会到我对共同体有用的时候才能够感觉到自己的价值。感觉到自己有价值之后才能有勇气。
28.闲居在家的孩子吃完饭之后帮忙洗碗。如果说 这种事就算了，快去上学吧。那就是按照理想的孩子的形象做减法运算的父母的话。就样做会更加挫伤孩子的勇气。
如果真诚说声谢谢的话，孩子也许就可以体会到自己的价值，进而迈出新的一步。
29.横向关系并不是说将任何人都变成朋友或者像对待朋友一样去对待每一个人，重要的是意识上的平等以及坚持自己应有的主张。
30.把对自己的执著转换成对他人的关心，建立起共同体感觉。从以下三点做起。
自我接纳:重要的不是被给予了什么，而是如何去利用被给予的东西。没必要特别积极的肯定自己，而是自我接纳。做不到就诚实地接受这个做不到的自己，
然后尽量朝着能够做到的方向去努力，不对自己撒谎。要分清能够改变的和不能改变的，不去关注无法改变的，而是去关注可以改变的。
...
31.人际关系不顺利既不是因为口吃也不是因为脸红恐惧症，真正的问题在于无法做到自我接纳、他者信赖和他者贡献，却将焦点聚焦到微不足道的一个方面并企图
以此来评价整个世界。这就是缺乏人生和谐的错误生活方式。
32.判断你的贡献是否起作用的不是你，那是他人的课题，是你无法干涉的问题。是否真正作出了贡献，从原理上根本无从了解。也就是说，进行他者贡献时候
的我们即使作出看不见的贡献，只要能够产生我对他人有用的主观感觉即贡献感出可以。
33.无论是希望特别优秀还是希望特别差劲，其目的都一样，引起他人的关注、脱离普通状态，成为特别的存在。这就是他们的目的。就是廉价的优越性追求。
34.要认真的活在此时此刻，正因为把模糊而微弱的光打向人生整体，所以才会感觉能看到过去和未来。但是，如果把强烈的聚光灯对准此时此该，那就会既看不
到过去也看不到未来。我们应该更加认真地过好此时此刻，如果感觉能够看到过去也能预测未来，那就证明你没有认真地活在此时此刻，而是生活在模糊而微弱的光中。
人生是连续的刹那，根本不存在过去和未来。你是想通过关注过去和未来为自己寻找免罪符。过去发生了什么与你的此时此刻没有任何关系，未来会如何也不是此时此刻
要考虑的问题。假如认真地活在此时此刻，那就根本不会说出那样的话。
35.人生就像是在每一个瞬间不停旋转起舞的连续的。并且，蓦然四顾时常常会惊觉，已经来到这里了？在跳着小提琴之舞的人中可能有人成为专业手，在跳着司法
考试之舞的人中也许有人成为律师，也可能有着截然不同的结果。但是，所有的人生都不是终结在路上，只要跳着舞的此时此刻充实就已经足够。
36.此时此刻是认真而谨慎地做好现在能做的事情。
37.为遥远的将来设定一个目标，并认为现在是其准备阶段。一直想着真正想做的是这样的事情，等时机到了就去做，是一种拖延人生的生活方式。只要在拖延人生，
我们就会无所进展，只能每天过着枯燥乏味的单调生活。因为在这种情况下，人就会认为此时此刻只是准备阶段和忍耐阶段。但是，为了遥远将来的考试而努力学习的
此时此刻却是真实存在。
38.人生中最大的谎言就是不活在此时此刻。纠结过去、关注未来，把微弱而模糊的光打向人生整体，自认为看到了些什么。你之前就一直忽略此时此刻，只关注根本
不存在的过去和未来。对自己的人生和无可替代的刹那撒了一个大大的谎言。
39.人生的意义是由你自己赋予自己的。人生没有普遍性的意义。但是，你可以赋予这样的人生以意义，而能够赋予你的人生以意义的只有你自己。


poco
1.children() 获取所有子级，操作对象是父级,返回的对象可以用[]取
"android.widget.GridView/"  表示父级
"android.widget.GridView/[0]",表示父级中的第一个子级，"/"号是级间的分隔号

2.poco("android.widget.GridView").child("android.view.View") 这个其实返回与children相同
poco("android.widget.GridView").child("android.view.View")[0]表示第一child中的元素

3.poco("star").exists() == poco(name="star").exists() 判断某个属性是否等于某值，没有设定属性默认是name，即这是判断是否有name:"star" 存在

4.poco()的操作为了生为一个ui对象
...
string[] currencyElement(const string& c) {

}

double currentPrice(const string& one, const string& two) {
...
html:
1.用来描述网页的一种标记语言，<html>,<head>等是标签，通常成对出现,
...
3.外部引用 - 使用外部 CSS 文件,样式会被很多个页面使用时，可用此方法
eg:
<head>
...
数据结构
1.堆 用数组实现二叉树,每节点是最大或最小，但右子节点和左子结点大小不定
最大堆:父节点比每个子节点大  最小堆：反之
2.用法：构建优先队列，支持堆排序，快速找出集合中的最大小值
3.与二叉搜索树的区别：
a.二中左子节点比右子节点小
