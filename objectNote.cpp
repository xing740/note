...
424.delete 的实现原理：
首先执行的析构函数，然后调用 operator delete 的标准库函数来释放所占的内存空间。
delete 用来释放单个对象所占的空间
...
426.字符串的子序列是原始字符串删除一些字符而不改变剩余字符相对位置形成的新字符串
...
1. curl
 ...
 启用禁用线路
{ data: { type:(trade. market ), lineid: xx, available: enable\ disable }}


加密解密

Token
1.相当于一个令牌，看到令牌后就不用作过多的认证。避免了服务器每次从数据库取玩家名、密码进行比较判断 
缺点:
1. token太长
2. 不太安全
3. 一旦生成无法让其失效，必须等到其过期才行，这样的话如果服务端检测到了一个安全威胁，也无法使相关的 token 失效。

不用session原因
1.服务器是单机工作没有问题，但有时会由负载均衡器(如 Nginx)来决定gac的请求由哪台机处理，某台机可能没有与gac建立连接，没有session，
又要通过共享session的方式解决

不安全
1.发到网络中的信息都可看成是不安全的，公开的。eg:服务器发送公钥给前端，公钥就是公开的。

md5:
1. 判断数据是否被改  
2. 数据 & 动态key = md5;, 动态key 前端自己生成，用公钥加密后发给后端，后端通过私钥解密得到动态key
3. 不直接加密信息，加密有规律的字符串可能会反推出加密方法

base64编码
1. 将非ASCII字符的数据转换成ASCII字符；
2、特别适合在http和mime协议下快速传输数据
3. 不是安全的加密方法，只是将明文显示成别人看不懂的字符格式
