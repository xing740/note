...
隐藏：发现在基类和派生类之间，派生类会隐藏基类中与其同名的函数,前提是基类中的函数没有被virtual修饰。如果被修饰了，但派生类中有一个同名的，返回值或参数不同，基类中的virtaul函数会被隐藏

418.指针在32位中是4byte,在64位中是8
...
define：在预处理阶段处理，没有类型，不会进行数据检查，代码中有多少处使用，就有多少替换，占用了代码段空间。不能进行调试(不能断点在define定义处，替换处也看不到替换后的代码)
...
new申请得到的内在自己存储区的，但不很多编译器的new底层是malloc，怎么是从堆中取的内存，又因为可以重载new，使其从其它地方申请内存，所以自由存储区不一样是堆。
