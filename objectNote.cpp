...
例：1
A a1 = GetA();a1是左值，GetA()先构造出一个临时对象，再调用A的拷贝赋值函数进行构造a1，临时对象再调用析构函数进行析构
...
read的过程：
read时，会调用底层的api(就是系统调用),这时会转为内核态，1.内核会负责将网卡中的数据复制到内核空间(需要一个过程,因为可能数据
...
class A : public y, public z {}; //因为x是被虚继承，所以x,y,z都共享一个x实例，所以A size = x大小1 + y内虚指针4 + z内的虚指针4，边界调整 12
...
1.如果将 a = b;则发现的是拷贝构造，默认的构造函数是位拷备。并不存在指针类型的转换，所以不存在多态，且这么简单的将b中虚函数表的指针赋值给
a 的虚函数表的指针，a调用虚函数时就会出错，所以编译器会对这种情况进行处理，赋值后，虚函数表的指针指向A,调用时打针出 "is A"
2.指针或引用类型进行赋值时， A* a = b; 则地址会进行类型转换，b中原本的虚函数指针，会被转成A类型的指针，所以只能寻址到A类虚函数表中存在
的函数。  下列16的解释应该也正确
